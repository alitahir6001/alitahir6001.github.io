// Console Dossier — shared theme, global CSS, reveal-on-scroll hook, and shared
// chrome (status bar, channel nav, foot). Locked to OXBLOOD — the warm-black /
// cream / oxblood dossier. Rebuilt for air: a generous vertical rhythm (driven by
// data-space), a dim "lounge" oxblood glow pooling behind the page, and calmer
// framing (data-frame). The console vocabulary stays — it just has room now.

const OXBLOOD = {
  id: 'oxblood',
  vars: {
    '--bg': '#08080a', '--bg-2': '#0d0d10',
    '--paper': '#d8d4cb', '--paper-2': '#9a958a', '--paper-3': '#5b574f', '--paper-4': '#2c2a26',
    '--rule': '#1a1a1e', '--rule-2': '#232227',
    '--accent': '#a32733', '--accent-deep': '#5b1820', '--accent-ink': '#c34050',
  },
};

// Reveal-on-scroll. Deterministic rect-based check (IntersectionObserver is
// unreliable inside sandboxed preview iframes). Runs on load + scroll + resize,
// only ever ADDS the class, so revealed content never flickers back out.
function useReveal(dep) {
  React.useEffect(() => {
    ...
    const t2 = setTimeout(reveal, 320);
  ...
  return (
    <React.Fragment>
      ...
      <nav className="cd-nav">
        ...
        {CD_NAV.map((n) => (
          <a key={n.key} className={cur === n.key ? 'cur' : ''} href={n.href}>{n.label}</a>
        ))}
      </nav>
    </React.Fragment>
  );
