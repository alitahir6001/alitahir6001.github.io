// --- Element Variables ---
    ...
    function validateInputLength(value, max = 120) {
...
async function downloadSyllabusAsPDF() {
    ...
    if (data.prerequisites?.length) {
        ...
        const firstItemHeight = getTextBlockHeight(`• ${data.prerequisites[0]}`, FONTS.BODY, usableWidth, 10);
        addPageIfNeeded(headerHeight + firstItemHeight);
        
        drawHeader('Prerequisites', FONTS.H1);
        
        data.prerequisites.forEach(p => {
            const itemHeight = getTextBlockHeight(`• ${p}`, FONTS.BODY, usableWidth, 10);
            addPageIfNeeded(itemHeight); // Check for each item
            drawTextBlock(`• ${p}`, FONTS.BODY, COLORS.SUBTLE, 10);
        });
        y += BLOCK_SPACING;
    }
    ...
    data.modules.forEach(module => {
        ...
        module.topics.forEach(topic => {
            ...
            if (topic.resources?.length) {
                ...
                const resFirstItemHeight = getTextBlockHeight(`• ${topic.resources[0]}`, FONTS.BODY, usableWidth, 20);
                ...
                topic.resources.forEach(r => {
                    const resItemHeight = getTextBlockHeight(`• ${r}`, FONTS.BODY, usableWidth, 20);
                    addPageIfNeeded(resItemHeight); // Check for each resource
                    drawTextBlock(`• ${r}`, FONTS.BODY, COLORS.SUBTLE, 20);
                });
            }
        });
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (!validateInputLength(sanitizedSubject, 120)) {
                throw new Error('Please provide a subject between 1 and 120 characters.');
            }
...
// Wire example buttons without inline onclick attributes
document.querySelectorAll("[data-example]").forEach(btn => {
  btn.addEventListener("click", () => fillExample(btn.getAttribute("data-example")));
});
